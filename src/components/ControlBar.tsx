...
export default function ControlBar({ paused, onPauseToggle, onCancel, onMinimize }: Props) {
  return (
    <div className="control-bar">
      <button className="ctrl" onClick={onPauseToggle} aria-label={paused ? 'Resume' : 'Pause'} title={paused ? '재시작' : '일시정지'}>
        {paused ? (
      ...
      <button className="ctrl" onClick={onMinimize} aria-label="Minimize" title="최소화">
        <svg viewBox="0 0 24 24" width="14" height="14"><circle cx="12" cy="12" r="6" fill="none" stroke="currentColor" strokeWidth="2"/></svg>
      </button>
      <button className="ctrl danger" onClick={onCancel} aria-label="Cancel" title="취소 / 종료">
